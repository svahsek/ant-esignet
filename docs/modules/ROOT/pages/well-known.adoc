...
e-Signet uses the "`.well-known`" directory to serve the following
purposes:
...
makes it easier for developers and web clients using e-Signet to know
...
developers using e-Signet can ensure interoperability with various web
standards and protocols. For example, e-Signet shares the context file
...
MOSIP’s "`.well-know`" directory contains three files, ++*++ jwks.json
++*++ openid-configuration ++*++ oauth-configuration
